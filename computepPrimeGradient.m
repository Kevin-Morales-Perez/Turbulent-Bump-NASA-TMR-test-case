function [grad_p_prime] = computepPrimeGradient(p_prime,wlsqOperator,nx,ny)
...
for j=2:nx-1
    ...
    %get central and neighborhood nodes;
    p_w=p_prime(i,j-1);
    p_n=p_prime(i,j);%Neumman condition
    p_e=p_prime(i,j+1);
    p_s=p_prime(i+1,j);
    p_p=p_prime(i,j); 
...
for j=2:nx-1
    ...
    p_n=p_prime(i-1,j);
    p_e=p_prime(i,j+1);
    p_s=0;% Fixed velocity or no mass flux , no pressure correction needed
    p_p=p_prime(i,j); 
