...
%VERY GOOD results with this mesh
...
domHgt=2;                            %Domain Height (m)
domLgt=4;                          %Domain Length (m)
...
if (distPlat+ bumpLgt)<domLgt
    ...
    nx=100;% number of cells in x axis 
    ny=50;% number of cells in y axis
    ncells=nx*ny; %Total cells

    est_wall_d=5e-4;%4.1750e-04;%Estimated wall distance (Height of the first cell 8.5e-6 for y+=1)
    %
    ...
    %  domain length
    fpRatio = 0.55;%665;%bumpLgt/domLgt;% Now the plate should contain half o f the 
    %total cells in X
    ...
    dx_1=0.007;%Lenght of the first cell at the leading and trailing edge 0.006
    grthFx=1.3;% Geometrical growth  Factor
    ninfLx=5;%Cells in the l/t edges side of the inflation layers
    ...
    dy_1=est_wall_d;% Height of the first cell (m);0.00021,0.0001;%
    grthF=1.105;% Geometrical growth  Factor ,1.3
    ninfL=10;% Cells in the boundary inflation layer
    %est_wall_d*((1-grthF^ninfL)/(1-grthF)) Boundary layer height
    ...
    for j=1:nx+1 %iterate over columns 
        ...
        dy_2=dy_2*1.6; %increment max dy in the inf layer if needed
    ...
    %_____________Modifying the grid to have a greater y grid spacing 
    %in the free flow zones
    %m_new=linspace(0.0075,0.00001,50);%New Slope parameter
    m_new=zeros(1,ny);
    %m_new=linspace(0.01,0.0002,ny);
    %UPWIND ZONE
    for j=nx_upstr:-1:1
        %compute x grid spacing
        dx_slp=X(i,j+1) -X(i,j);
        for i=ny:-1:2
            Ytest(i,j)=Ytest(i,j+1) + dx_slp*m_new(i);
        end
    end 

    %DOWNWIND ZONE
    for j=nx-nx_dwnstr + 2:1:nx+1
        %compute x grid spacing
        dx_slp=X(i,j) -X(i,j-1);
        for i=ny:-1:2
            Ytest(i,j)=Ytest(i,j-1) + dx_slp*m_new(i);
        end
    end
    ...
    solidMask = false(1, nx);
    solidMask(nxSolid) = true; 

    %Kz factors for second order Dirichlet Boundary Conditions

    %pressure
    G_2=X(1,2)/(X(1,3) -X(1,2));
    kz2=1 + 1/(2*G_2);
