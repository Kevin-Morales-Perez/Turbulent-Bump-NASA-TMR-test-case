...
if (distPlat+ bumpLgt)<domLgt
    ...
    nx=160;% number of cells in x axis 
    ny=45;% number of cells in y axis
    ncells=nx*ny; %Total cells

    est_wall_d=1.7000e-05;%6.8e-5;%4.1750e-04;%Estimated wall distance 
    % (Height of the first cell 8.5e-6 for y+=1)
    ...
    %  domain length
    fpRatio = 0.625;%bumpLgt/domLgt;% Now the plate should contain half o f the 
    %total cells in X
    ...
    dx_1=0.005;%Lenght of the first cell at the leading and trailing edge
    grthFx=1.4;% Geometrical growth  Factor
    ninfLx=4;%Cells in the l/t edges side of the inflation layers
    ...
    grthF=1.3;% Geometrical growth  Factor ,1.3
    ninfL=15;% Cells in the boundary inflation layer

    s_99=dy_1*((1-(grthF^ninfL))/(1-grthF));
    ...
    %m_new=zeros(1,ny);
    m_new=linspace(4e-4,7.5556e-06,ny);
    %UPWIND ZONEY
    for j=nx_upstr:-1:1
