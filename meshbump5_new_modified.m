...
if (distPlat+ bumpLgt)<domLgt
    ...
    nx=165;% number of cells in x axis 
    ny=40;% number of cells in y axis
    ncells=nx*ny; %Total cells

    est_wall_d=1.7e-5;%6.8e-5;%4.1750e-04;%Estimated wall distance 
    ...
    %  domain length
    fpRatio = 0.545;%bumpLgt/domLgt;% Now the plate should contain half o f the 
    %total cells in X
    ...
    grthF=1.38;% Geometrical growth  Factor ,1.3
    ...
    %m_new=zeros(1,ny);
    m_new=linspace(4.5e-4,7.5556e-6,ny);
    %UPWIND ZONE
    for j=nx_upstr:-1:1
