...
%ALGORITHM: SIMPLE
%CONVECTIVE SCHEME: UPWIND_MUSCL (MOMENTUM) UPWIND 1ST ORDER (TURBULENCE)
...
%u0=15;                    %Velocity at the inlet (m/s) 
...
alpha_uv=0.3;%0.1925;  %<---------------------     x-y momentum 
alpha_p=0.28; %<---------------------     pressure correction
alpha_nt=1; %<---------------------     nu-tilde
...
max_iterations_u=0;% Max iterations for momentum eqs.
max_iterations_v=0;% Max iterations for momentum  y eq .
max_iterations_p=100;% Max iterations for pressure eq.
max_iterations_nt=1;% Max iterations for SA equation.
...
tvd_mode="ENABLED";
tvd_iterations=100;%Inner TVD iterations 
...
f_ramp=1;%.00001126697361231166;%Ramping factor
ramping_steps=29128;% Number of steps to ramp
ramping_spacing=1;
...
while convergedFlg==false
    ...
    if iterations_cont>=iterations_trig
        ...
        if mod(iterations_cont,20000) == 0
            file_var_name="solvingRe3M_mesh9_ItNum_" + string(iterations_cont) + ".mat";
            save(file_var_name)
        end
        
   
        %Change pressure correction solver
