function [ap_W,ap_N,ap_E,ap_S,ap_P,suP] = pressureCorr_link_coeff(u_face,...
 ...
 %nx_dwnstr: number of cells in the free downstream zone
   ...
   %________________________EDGES_________________________
    ...
    for j=2:nx-1
        ...
        ap_S(i,j) = (fAs/dist_s)*((volP/aP(i,j))*weight_s  + ... 
            (1-weight_s)*(volS/aP(i+1,j))) -...
            ((volP*fAn)/(aP(i,j)*fAe))*(1-weight_s);
    ...
    for i=2:ny-1
        ...
        ap_W(i,j) = (fAw/dist_w)*((volP/aP(i,j))*weight_w  +...
            (1-weight_w)*(volW/aP(i,j-1))) ...
            -((volP*fAe)/(aP(i,j)*fAs))*(1-weight_w);
    ...
    %calculate pressure correction coeffitient for central cell
    ap_P(i,j)= ap_E(i,j) + ap_S(i,j) + ((volP*fAn)/(aP(i,j)*fAe));
    ...
    %calculate pressure correction coeffitient for central cell
    ap_P(i,j)= ap_W(i,j) + ap_S(i,j) + ((volP*fAe)/(aP(i,j)*fAs)) ...
        + ((volP*fAn)/(aP(i,j)*fAe));
    ...
    %calculate pressure correction coeffitient for central cell
    ap_P(i,j)= ap_W(i,j) + ap_N(i,j) + ((volP*fAe)/(aP(i,j)*fAs));
