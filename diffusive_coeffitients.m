function [dW,dN,dE,dS,dW_c,dN_c,dE_c,dS_c] = ...
    diffusive_coeffitients(distNeighbNods,uVecNormFaces,...
    uVecNeighbNods,lgtFaces,uVecParlFaces,dW,dN,dE,dS,dW_c,dN_c,...
    dE_c,dS_c,nx,ny)
...
for i=1:ny
    for j=1:nx
        ...
        %Compute operator
        dW(i,j)=(lgt_fw/(dist_p_w*dot(un_fw,uv_pw)));
...
for i=1:ny
    for j=1:nx
        ...
        %Compute operator
        dN(i,j)=(lgt_fn/(dist_p_n*dot(un_fn,uv_pn)));
...
for i=1:ny
    for j=1:nx
        ...
        %Compute operator
        dE(i,j)=(lgt_fe/(dist_p_e*dot(un_fe,uv_pe)));
...
for i=1:ny
    for j=1:nx
        ...
        %Compute operator
        dS(i,j)=(lgt_fs/(dist_p_s*dot(un_fs,uv_ps)));
...
for i=1:ny
    for j=1:nx
        ...
        dW_c(i,j) = -(dot(up_fw,uv_pw)/dot(un_fw,uv_pw));
...
for i=1:ny
    for j=1:nx
        ...
        %Compute operator for cross diffusion terms
        dN_c(i,j) = -(dot(up_fn,uv_pn)/dot(un_fn,uv_pn));
...
for i=1:ny
    for j=1:nx
        ...
        %Compute operator for cross diffusion terms
        dE_c(i,j) = -(dot(up_fe,uv_pe)/dot(un_fe,uv_pe));
...
for i=1:ny
    for j=1:nx
        ...
        %Compute operator for cross diffusion terms
        dS_c(i,j) = -(dot(up_fs,uv_ps)/dot(un_fs,uv_ps));
